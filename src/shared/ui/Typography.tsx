...
export const Typography: FC<Props> = function Typography(props) {
  ...
  let styleSize = STYLE_SIZE[size];
  let styleWeight = STYLE_WEIGHT[weight];
  let styleAlign = STYLE_ALIGN[align];

  return (
    <p
      {...rest}
      className={`${rest.className} ${styleWeight} ${styleAlign} ${color} ${styleSize}`}
    >
      {rest.children}
    </p>
  );
